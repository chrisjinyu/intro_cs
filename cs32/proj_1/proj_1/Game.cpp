#include "Game.h"			//include its own header

#include "Arena.h"			//required to call Arena object functions
#include "Player.h"			//required to call Player object functions

#include "globals.h"		//need globals to access MAX constants
#include <iostream>			//required to cout/cin to run game
...
string Game::takePlayerTurn()
{
	for (;;)
	{
		bool isHist = false;				//flag used to prevent extra cout at the end as if user typed in incorrect

		cout << "Your move (n/e/s/w/h/x or nothing): ";			//adjusted to display h as a new key option
		string playerMove;
		...
		{
			if (tolower(playerMove[0]) == 'h')			//History implementation using 'h' character
			{
				isHist = true;							//set flag to true, so that the message at the end does not appear as if user incorrectly implemented
				m_arena->history().display();			//executes the display function of the Arena's History object
				cout << "Press enter to continue." << endl;
				cin.ignore(10000, '\n');				//wait for '\n' character to be input
				m_arena->display("");					//display the game board again with no message
			}
		...
		if(!isHist)		//prevents the history implementation from giving the following error message
...
//AUXILIARY FUNCTIONS, UNIQUE TO GAME
