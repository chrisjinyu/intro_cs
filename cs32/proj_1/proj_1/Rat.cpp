#include "Rat.h"			//header
#include "Arena.h"			//completes Arena class

#include "globals.h"		//required to refer to global constants
#include <iostream>	
	...
	if (attemptMove(*m_arena, randInt(0, NUMDIRS - 1), m_row, m_col))
	{
		if (m_arena->getCellStatus(m_row, m_col) == HAS_POISON)
		{
			...
			if(m_health == INITIAL_RAT_HEALTH)
				m_arena->history().record(m_row, m_col);			//records to the m_arena m_history object
			m_health--;
		}
	}
